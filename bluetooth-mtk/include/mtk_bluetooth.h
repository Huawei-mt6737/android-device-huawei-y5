...
// to avoid change the bluetooth HAL(bt_sdp.h), we define the include guards as ANDROID_INCLUDE_BLUETOOTH_H
// it's the same with the include guard in the file bluetooth.h
...
typedef struct {
    ...
    /**
     * enable/disable ssp debug mode
     */
    int (*config_ssp_debug_mode)(bool enable);
} bt_interface_t;
